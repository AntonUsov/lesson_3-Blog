    ...
    def new_commit_list(self, log):
        ...
        for line in text:
            if 'revno:' in line and 'committer: user_' in line and 'timestamp:' and '' in line:
                ...
                revno = data[1]
                if '[' in revno:
                    stop_index = revno.index('[') -1
                    revno_value = revno[7:stop_index]
                else:
                    revno_value = revno[7:]

                comitter = data[2]
                comitter_value = comitter[11:]

                timestamp = data[3]
                timestamp_value = timestamp[15:34]

                slovar = {
                    'username': comitter_value,
                    'timestamp': timestamp_value,
                    'revno': revno_value
                }
                all_commits.append(slovar)
